...
#[derive(Clone)]
struct SLList {
    data: Vec<String>, // храним значения в векторе для простоты
}
...
impl SLList {
    ...
    fn save_to_file(&self, filename: &str) {
        ...
        let mut file = File::create(filename)
            .unwrap_or_else(|_| panic!("Cannot open list file for writing: {}", filename));
...
fn main() {
    ...
    let mut list = SLList::load_from_file(&filename);

    let parts: Vec<&str> = query.split_whitespace().collect();
    match parts[0] {
        "SLADDHEAD" => {
            if parts.len() < 2 {
                eprintln!("Error: SLADDHEAD requires a value");
                return;
            }
        ...
        "SLADDTAIL" => {
            if parts.len() < 2 {
                eprintln!("Error: SLADDTAIL requires a value");
                return;
            }
        ...
        "SLSEARCH" => {
            if parts.len() < 2 {
                eprintln!("Error: SLSEARCH requires a value");
                return;
            }
